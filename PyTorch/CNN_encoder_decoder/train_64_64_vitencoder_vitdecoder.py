...
vit_encoder = ViTModel(configuration)
...
class recon_model(nn.Module):
    def __init__(self, vit_encoder, vit_I, vit_phi):
    ...
    def forward(self, x):
        #encoder
        x = self.vit_encoder(x).last_hidden_state[:, 0:16, :][:, None, :, : ]
...
# check model
model = recon_model(vit_encoder, vit_I, vit_phi)
